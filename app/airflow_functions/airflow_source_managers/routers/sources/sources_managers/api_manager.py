...
from airflow_functions.airflow_source_managers.routers.sources.sources_managers.api_manager_utils.custom_authentications import (  # noqa
    HTTPTokenAuth,
    HTTPAPIkeyAuth,
...
from airflow_functions.airflow_source_managers.routers.sources.models.api_model import (
    APIAuthType,
    validate_restapi_auth_data_depending_on_auth_type,
)
from airflow_functions.airflow_source_managers.routers.sources.sources_managers.api_manager_utils.utils import (
...
class APISourceManager(ABCSourceManager):
    ...
    def __clear_auth_type(auth_type):
        ...
        if auth_type not in auth_types:
            raise ValueError(f"Source.auth_type must be one of {auth_types}")
        return auth_type
    ...
    def obj_name_to_load_from_response(self, value):
        if value is not None and isinstance(value, str) is False:
            raise ValueError(
                "ManualFileSourceManager.obj_name_to_load_from_response must be None or str type"
            )
    ...
    def source_data_columns(self, value):
        if value is not None and isinstance(value, list) is False:
            raise ValueError(
                "ManualFileSourceManager.source_data_columns must be None or list type"
            )
    ...
    def query_params(self, value):
        if isinstance(value, dict) or value is None:
            ...
            raise TypeError("query_params must be None or be instance of dict")
    ...
    def body_params(self, value):
        if isinstance(value, dict) or value is None:
            ...
            raise TypeError("body_params must be None or be instance of dict")
    ...
    def get_pandas_data_frame_based_on_response(
        ...
        if res_type == RESTAPIResponseTypes.LIST_OF_VALUES.value:
            df = pd.DataFrame(response.json())
        elif res_type == RESTAPIResponseTypes.LIST_OF_OBJECTS.value:
            df = pd.DataFrame(response.json())
        elif res_type == RESTAPIResponseTypes.OBJECT.value:
            df = pd.DataFrame([response.json()])
        elif res_type == RESTAPIResponseTypes.FILE.value:
