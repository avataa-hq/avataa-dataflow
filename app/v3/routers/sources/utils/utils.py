...
async def check_group_exists(session: AsyncSession, group_id: int):
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Source group with id={group_id} does not exist!",
        )
...
async def check_source_name_in_group_exists(
    session: AsyncSession, group_id: int, source_name: str
):
    stmt = select(Source.id).where(
        Source.group_id == group_id, Source.name == source_name
    )
    ...
    if res is not None:
        raise HTTPException(
            status_code=422,
            detail=f"Source named '{source_name}' already exists in group with id={group_id}!",
        )
...
async def check_source_exists(session: AsyncSession, source_id: int):
    ...
    if res is None:
        raise HTTPException(
            status_code=404,
            detail=f"Source with id={source_id} does not exist!",
        )
    return res


async def update_for_simple_types(
    source_id: int,
    source_model: Union[
        SFTPModelCreate,
        FTPModelCreate,
    ...
    if source_from_db is not None:
        raise HTTPException(
            status_code=422,
            detail=f"Source named '{source_model.name}' already exists in group with id={source_model.group_id}!",
        )
