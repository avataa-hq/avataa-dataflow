...
def validate_restapi_auth_data_depending_on_auth_type(auth_type, auth_data):
    match auth_type:
        case APIAuthType.APIKEY.value:
            APITokenAuthModel.validate(auth_data)

        case APIAuthType.MULTIAPIKEY.value:
            APIMultiKeyModel.validate(auth_data)

        case APIAuthType.BASIC.value:
            APIBaseAuthModel.validate(auth_data)

        case APIAuthType.DIGEST.value:
            APIBaseAuthModel.validate(auth_data)

        case APIAuthType.TOKEN.value:
            APITokenAuthModel.validate(auth_data)

        case APIAuthType.OPENID.value:
            APIOpenIDAuthModel.validate(auth_data)

        case APIAuthType.NOAUTH.value:
            pass
        case _:
